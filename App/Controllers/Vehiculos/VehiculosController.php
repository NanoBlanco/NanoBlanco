...
require_once ROOT.MODL .'Vehiculos/VehiculosModel.php';
...
class VehiculosController extends Controller
{
    ...
    */
    public function show($view='',$alert='')
    {
        ...
        $vehiculos = $this->model->todos();
        $ubicaciones = $this->model_ubicacion->todos();
        $this->render(__CLASS__, $view, array('items'=>$items, 'ubicaciones'=>$ubicaciones, 'vehiculos'=>$vehiculos, 'alert'=> $alert)); 
    }
    ...
    public function nuevoVehiculo()
    {
        ...
        $ubicaciones = $this->model_ubicacion->todos();
        $view='Crear';
        $this->render(__CLASS__, $view, array('vehiculos'=>$vehiculos, 'items'=>$items, 'ubicaciones'=>$ubicaciones));
        exit();
    }

    public function editarVehiculo() {
        if (isset($_POST["id"]) && !empty($_POST["id"])) {
            $vehiculo = $this->model->porId($_POST['id']);
            $ubicaciones = $this->model_area->todos();
            $view='Editar';
            $this->render(__CLASS__, $view, array('vehiculo'=>$vehiculo,'ubicaciones'=>$ubicaciones));
        }
        exit();
    }
    ...
    public function eliminarVehiculo()
    {
        ...
        header('location: ../Vehiculos/?alert='.$alert);
    }
}